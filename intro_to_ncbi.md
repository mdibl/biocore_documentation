...
NCBI integrates relevent data between databases, meaning if you are examining a Gene Expression Omnibus (GEO) Dataset, a link to the appropriate Profile page is included.

As this is just an introduction to NCBI, a guide to building queries with Entrez will follow in a seperate tutorial.
...
* Largest publicaly available repository of High Throughput Sequencing (HTS) data
...
#### Additonally, there are suites of tools for general data access (Entrez Programming Utilities) and ones that are repository specific (SRA Toolkit, GEO2R), though some require software to be installed locally before use.
